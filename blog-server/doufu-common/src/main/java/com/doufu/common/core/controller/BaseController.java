package com.doufu.common.core.controller;
...
import com.doufu.common.utils.PageUtils;
...
public class BaseController {
     ...
     * 获取登录用户名
     */
    public Set<Long> validatedRemoveIds(Set<Long> idList) {
        if (CollectionUtils.isEmpty(idList)) {
            throw new WarnException("请正确的填写id");
        }
        int size = idList.size();
        if (size > 10) {
            throw new ServiceException("一次最多只能删除10个");
        }
        return idList.stream().filter(id -> String.valueOf(id).length() < 20 && !String.valueOf(id).isEmpty()).collect(Collectors.toSet());
    }
}
